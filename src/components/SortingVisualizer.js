...
const SortingVisualizer = () => {
  ...
  const swappingAnimations = (animations) => {
    for (let i = 0; i < animations.length; i++) {
      ...
      if (animations[i][0] == null) continue;
      if (isColorChange) {
        const [barOneIdx, barTwoIdx] = animations[i];
        const barOneStyle = arrayBars[barOneIdx].style;
        const barTwoStyle = arrayBars[barTwoIdx].style;
  ...
  const merge_sort = () => {
    if (started === false) setStarted(true);
  ...
  const sort = () => {
    ...
    if (type === "") {
      ...
      setTimeout(() => {
        if (type === "merge_sort") merge_sort();
        else if (type === "bubble_sort") bubble_sort();
        else if (type === "insertion_sort") insertion_sort();
        else if (type === "quick_sort") quick_sort();
        else heap_sort();
      }, 10);
    }
  };
    ...
    <div>
      ...
      <div className="App">
        <div className="row">
          ...
          <button id="sort" className="sortbutton" onClick={sort}>
            Sort
          </button>
        </div>
        ...
        <div id="box" className="Box">
          {arr.map((value, index) => {
            return (
              <div className="bar" style={{ height: value }} key={index}></div>
            );
          })}
        </div>
      </div>
